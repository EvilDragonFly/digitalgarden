---
{"dg-publish":true,"permalink":"/career/cpp八股文面试/","hide":true,"hideInGraph":true,"noteIcon":"3"}
---


[[career/cpp面试问题汇总/数据结构\|数据结构]]
#interview #c #cpp
...
[maclloc and new](https://blog.csdn.net/leikun153/article/details/80612130)

* malloc/free(library function)和new/delete(operator)都是用来申请内存和回收内存的。
* 在对非基本数据类型的对象使用的时候，对象创建的时候还需要执行构造函数，销毁的时候要执行析构函数。而malloc/free是库函数，是已经编译的代码，所以不能把构造函数和析构函数的功能强加给malloc/free。
* malloc/free是分配一段内存区域，但是new/delete是针对一个对象
### （6） C和C++的区别
包括但不限于：
...
### （11） define 和const的联系与区别（编译阶段、安全性、内存占用等）
  ...
  区别：
* define定义的常量没有类型，只是进行了简单的替换，可能会有多个拷贝，占用的内存空间大，const定义的全局常量是有类型的，存放在静态存储区，只有一个拷贝，占用的内存空间小。
* define定义的常量是在预处理阶段进行替换，而const在编译阶段确定它的值。
* define不会进行类型安全检查，而const会进行类型安全检查，安全性更高。
* const可以定义函数而define不可以。
* 普通的局部变量被const修饰后仅仅意味着在表达式上不能显式地改变该变量值，否则编译器会报语法错误，但该变量仍存放在栈区
...
* _unordered_map是使用哈希实现的，占用内存比较多，查询速度比较快，是常数时间复杂度。它内部是无序的，需要实现\=\=操作符。
...
### （22） C++中的重载和重写的区别：
* 重载(overloaded)是指函数名相同，参数列表不同的函数实现方法。它们的返回值可以不同，但返回值不可以作为区分不同重载函数的标志。
* 覆盖(overridden)是指函数名相同，参数列表相同，只有方法体不相同的实现方法。一般用于子类继承父类时对父类virtual function的重写。子类的同名方法屏蔽了父类方法的现象称为隐藏。
* 重写(overwite/redefined) 函数名相同，参数列表相同，方法体不相同的实现方法。子类继承父类时对于父类的非virtual function的重写。
* [详情](https://stackoverflow.com/questions/26743991/redefining-vs-overriding-in-c)
...
[常量存储区与静态存储区的区别](https://www.cnblogs.com/yanqi0124/p/3795547.html)
[c++ memory layout](https://blog.gtwang.org/programming/memory-layout-of-c-program/)
...
* 常量存储区，这是一块比较特殊的存储区，他们里面存放的是常量字符串，不允许修改，更改会报。
```c
...
* 编译时多态: 模板和函数重载, 但是模板不能分离编译(模板函数不可能是虚函数)
* 运行时多态: 虚函数：在基类的函数前加上virtual关键字，在派生类中重写该函数，运行时将会根据对象的实际类型来调用相应的函数。如果对象类型是派生类，就调用派生类的函数；如果对象类型是基类，就调用基类的函数.（通过虚函数表和虚函数指针实现，运行开销比较大）
...
* weak_ptr是为了协助shared_ptr而出现的。它不能访问对象，只能通过lock或者expire方法观测shared_ptr的引用计数，防止出现死锁，防止循环引用问题。
### （40） 调试程序的方法
* 通过设置断点进行调试
* 打印log进行调试
* 打印中间结果进行调试
...
### （43） 模板的用法与适用场景 实现原理
[模板与分离编译](https://www.cnblogs.com/downey-blog/p/9583769.html)
...
```
因为使用成员初始化列表进行初始化的话，会直接使用传入参数的拷贝构造函数进行初始化，省去了一次执行传入参数的默认构造函数的过程，否则会调用一次传入参数的默认构造函数,再在函数体中调用拷贝构造函数。所以使用成员初始化列表效率会高一些。<br>
...
* open hashing
	* 链式地址法(拉链法)
* closed hashing
	* open address(开放定址法):从发生冲突的那个单元起，按照一定的次序，从哈希表中中找到一个空闲的单元。开放定址法需要表长度要大于等于所需要存放的元素
		* 线行探查法
		* 平方探查法
		* 双散列函数探查法
	* 再哈希法
...
![三次握手.png](/img/user/career/cpp面试问题汇总/fig/三次握手.png)
* 四次挥手断开连接：
...
![四次挥手.png](/img/user/career/cpp面试问题汇总/fig/四次挥手.png)
...
![拥塞控制](/img/user/career/cpp面试问题汇总/fig/拥塞控制.png)
...
* 非对称加密：需要公钥和私钥，公钥用来加密，私钥用来解密
  ...
  * 常用算法：RSA，ECC，DSA
### （30） 数字证书的了解（高频）
![数字证书.jpg](/img/user/career/cpp面试问题汇总/fig/数字证书.jpg)
...
### （38） 服务器攻击（DDos攻击）
### （39）DNS的工作过程和原理
![](/img/user/career/cpp面试问题汇总/fig/DNS查询图解.png)
...
![(fig/网络协议层.png](/img/user/career/cpp面试问题汇总/fig/网络协议层.png)
...
### （9） B树和B+树的区别（重点）
![Bptree.png](/img/user/career/cpp面试问题汇总/fig/Bptree.png)
...
![select](/img/user/career/cpp面试问题汇总/fig/select.png)
...
![epoll](/img/user/career/cpp面试问题汇总/fig/epoll.png)
...
![管道通信.png](/img/user/career/cpp面试问题汇总/fig/管道通信.png)
...
* 三态模型
  ...
  3. 阻塞：有IO事件或者等待其他资源
  ![](/img/user/career/cpp面试问题汇总/fig/三态模型.png)
* 五态模型
  1. 新建态：进程刚刚创建。
  2. 就绪态：
  3. 运行态：
  4. 等待态：出现等待事件
  5. 终止态：进程结束
   ![](/img/user/career/cpp面试问题汇总/fig/五态模型.png)
...
* 七态模型
  ...
  6. 挂起等待态
  7. 终止态
![](/img/user/career/cpp面试问题汇总/fig/七态模型.png)
...
```
### （4） 二维码登录的实现过程 场景题
![](/img/user/career/cpp面试问题汇总/fig/二维码登录流程.png)
### （5） 不使用临时变量实现swap函数
...
![](/img/user/career/cpp面试问题汇总/fig/bitmap1.png)
...
![](/img/user/career/cpp面试问题汇总/fig/布隆过滤器.png)
...
![](/img/user/career/cpp面试问题汇总/fig/spark内存一.png)
